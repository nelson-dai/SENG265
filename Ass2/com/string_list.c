...
int StringList_Remove(StringList* L, char* str){
    ...
    while(curr->next != NULL){
         if(strcmp(curr->next->element,str) == 0){ //find the one just before the desired one
             
             StringListNode* deleteNode = curr->next;
             
             if(deleteNode->next == NULL){  //we are at tail
                 deleteNode->previous->next = NULL;
                 free(deleteNode->element);
                 free(deleteNode);
                 return 1;
             }
             curr->next = deleteNode->next;
             deleteNode->next->previous = curr;
             free(deleteNode->element);
             free(deleteNode);
             return 1;
         }
...
void StringList_RemoveNode(StringList* L, StringListNode* node){
    StringList_Remove(L,node->element); //hack ?
}
